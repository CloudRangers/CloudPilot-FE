...
export default function ManagerApprovalPage() {
  ...
  const [roleLabel, setRoleLabel] = useState("관리자님")
  const router = useRouter()

  // ✅ 초기 로드 시 역할 확인 및 데이터 로드
  useEffect(() => {
    const storedRole = localStorage.getItem("userRole")

    // 권한 확인
    if (!storedRole || !["HEAD", "ADMIN"].includes(storedRole)) {
      alert("접근 권한이 없습니다.")
      router.replace("/")
      return
    }
    ...
    // 역할별 호칭 설정
    setRoleLabel(roleLabelMap[storedRole] || "관리자님")
    loadRequests()
  }, [router])
  ...
  const loadRequests = () => {
    ...
    const managerRequests = storedRequests.filter(
      (req: PackageRequest) =>
        req.requesterRole === "team-leader" ||
        (req.requesterRole === "employee" && req.firstApprovalStatus === "approved"),
    )
  ...
  const handleApprove = () => {
    ...
    const updatedRequests = requests.map((req) =>
      req.id === selectedRequest.id
        ? {
            ...req,
            finalApprovalStatus: "approved" as const,
            finalApprovedDate: new Date().toISOString(),
          }
        : req,
    )
    ...
    notifications.push({
      ...
      type: "final_approval",
      title: "패키지 신청 최종승인",
      message: `${selectedRequest.packages.map((p) => p.name).join(", ")} 패키지 신청이 최종 승인되었습니다.`,
  ...
  const handleReject = () => {
    ...
    const updatedRequests = requests.map((req) =>
      req.id === selectedRequest.id
        ? {
            ...req,
            finalApprovalStatus: "rejected" as const,
  ...
  // ✅ 상태 필터링
  const pendingRequests = requests.filter((req) => req.finalApprovalStatus === "pending")
  const processedRequests = requests.filter((req) => req.finalApprovalStatus !== "pending")
          ...
          <div className="mx-auto max-w-6xl">
            ...
            <div className="space-y-3 text-center mb-8">
              ...
              <p className="text-muted-foreground">
                {roleLabel}, 패키지 신청을 검토하고 최종 승인해주세요
              </p>
            </div>
            ...
            <div className="space-y-8">
              <div>
                <h2 className="text-xl font-semibold mb-4 flex items-center gap-2">
                  ...
                  최종 승인 대기중 ({pendingRequests.length})
                </h2>
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-sm text-muted-foreground">
                                ...
                                {request.requesterRole === "team-leader" ? "팀장" : "사원"}
                              </span>
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    {processedRequests.map((request) => (
                      <Card
                        key={request.id}
                        className="border-2 shadow-lg overflow-hidden opacity-75"
                      >
                        ...
                        <div className="p-6">
                          ...
                          {request.finalApprovalStatus === "rejected" &&
                            request.finalRejectionReason && (
                              <div className="mt-4 rounded-lg bg-red-50 border border-red-200 p-3">
                                <p className="text-sm text-red-700">
                                  반려 사유: {request.finalRejectionReason}
                                </p>
                              </div>
                            )}
                        </div>
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          {selectedRequest && (
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="rounded-lg border bg-muted/30 p-4">
                  ...
                  <p className="text-sm">
                    ...
                    {selectedRequest.requesterRole === "team-leader" ? "팀장" : "사원"}
                  </p>
                  <p className="text-sm">
              ...
              {/* 1차 승인 날짜 */}
              {selectedRequest.firstApprovedDate && (
                <div className="rounded-lg bg-green-50 border border-green-200 p-3">
                  <p className="text-sm text-green-800">
                    <CheckCircle className="inline h-4 w-4 mr-1" />
                    1차 승인: {formatDate(selectedRequest.firstApprovedDate)}
                  </p>
                </div>
              )}
              ...
              {/* 반려 사유 입력 및 버튼 */}
              {selectedRequest.finalApprovalStatus === "pending" && (
                <>
                  <div className="space-y-2">
